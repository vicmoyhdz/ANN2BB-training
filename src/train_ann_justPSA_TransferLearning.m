...
function train_ann_justPSA_TransferLearning(varargin)
    %% *SET-UP*
    wd  = varargin{1};
    ann = varargin{2};
    dbn_name = varargin{3};
    ...
    [inp2.idx,tar2.idx] = trann_check_vTn(inp2,tar2,db2,1e-8);
    ...
    switch upper(ann.scl)
        ...
        case 'AB'
            ...
            ia1 = strcmpi('A',{db2.SIMBAD(:).site_EC8});
            ia2 = strcmpi('A*',{db2.SIMBAD(:).site_EC8});
            ia  = logical(ia1+ia2);
            ib1 = strcmpi('B',{db2.SIMBAD(:).site_EC8});
            ib2 = strcmpi('B*',{db2.SIMBAD(:).site_EC8});
            ib  = logical(ib1+ib2);
            idx2_cl = logical(ia+ib);
        case 'CD'
            ...
            ia1 = strcmpi('C',{db2.SIMBAD(:).site_EC8});
            ia2 = strcmpi('C*',{db2.SIMBAD(:).site_EC8});
            ia  = logical(ia1+ia2);
            ib1 = strcmpi('D',{db2.SIMBAD(:).site_EC8});
            ib2 = strcmpi('D*',{db2.SIMBAD(:).site_EC8});
            ib  = logical(ib1+ib2);
            idx2_cl = logical(ia+ib);
    end
    ...
    for i_=1:db.nr
     ...
     for i_=1:db2.nr
        db2.simbad(i_) = db2.SIMBAD(idx2_cl1(i_));
    end
    ...
    switch ann.cp
        ...
        case {'h12v'}
            ...
            for j_ = 1:db2.nr
                PSA2_1(j_,:) = db2.simbad(j_).psa_h1(:)';
                PSA2_2(j_,:) = db2.simbad(j_).psa_h2(:)';
                PSA2_3(j_,:) = db2.simbad(j_).psa_v(:)';
            end
        ...
        case {'h12'}
            ...
            % _HORIZONTAL COMPONENT BOTH (in the same branch)
        case {'h'}
            for j_ = 1:db.nr
                PSA_1(j_,:) = db.simbad(j_).psa_h1(:)';
                PSA_1(j_+db.nr,:) =db.simbad(j_).psa_h2(:)';
                % PGV(j_,:) = db.simbad(j_).pgv(1);
            end
            for j_ = 1:db2.nr
                PSA2_1(j_,:) = db2.simbad(j_).psa_h1(:)';
                PSA2_1(j_+db2.nr,:) = db2.simbad(j_).psa_h2(:)';
                % PGV2(j_,:) = db2.simbad(j_).pgv(1);
            end
        ...
        case {'h_inv'}
            ...
            for j_ = 1:db2.nr
                PSA2_1(j_,:) = [db2.simbad(j_).psa_inv(:)'];
                % PGV2(j_,:) = db2.simbad(j_).pgv(1);
            end
        ...
        case {'h2'}
            for j_ = 1:db.nr
                ...
                % PGV(j_,:) = db.simbad(j_).pgv(2);
            end
            for j_ = 1:db2.nr
                PSA2(j_,:) = db2.simbad(j_).psa_h2(:)';
                % PGV2(j_,:) = db2.simbad(j_).pgv(2);
            end
        case 'gh'
            for j_ = 1:db.nr
                PSA_1(j_,:) = geomean([db.simbad(1,j_).psa_h1(:)';...
                    db.simbad(1,j_).psa_h2(:)'],1);
                % PGV(j_,:) = geomean([db.simbad(j_).pgv(1);db.simbad(j_).pgv(2)],1);
            end
            for j_ = 1:db2.nr
                PSA2_1(j_,:) = geomean([db2.simbad(1,j_).psa_h1(:)';...
                    db2.simbad(1,j_).psa_h2(:)'],1);
                % PGV2(j_,:) = geomean([db2.simbad(j_).pgv(1);db2.simbad(j_).pgv(2)],1);
            end
            % _VERTICAL COMPONENT_
        case 'ud'
            for j_ = 1:db.nr
                PSA_1(j_,:) = db.simbad(j_).psa_v(:)';
                % PGV(j_,:) = db.simbad(j_).pgv(3);
            end
            for j_ = 1:db2.nr
                PSA2_1(j_,:) = db2.simbad(j_).psa_v(:)';
                % PGV2(j_,:) = db2.simbad(j_).pgv(3);
            end
    end
   ...
   if strcmp(ann.cp,'h12v')
        ...
        inp2.simbad_1  = -999*ones(inp.nT,db2.nr);
        tar2.simbad_1  = -999*ones(tar.nT,db2.nr);

        inp.simbad_2  = -999*ones(inp.nT,db.nr);
        tar.simbad_2 = -999*ones(tar.nT,db.nr);
        inp2.simbad_2  = -999*ones(inp.nT,db2.nr);
        tar2.simbad_2  = -999*ones(tar.nT,db2.nr);

        inp.simbad_5  = -999*ones(inp.nT,db.nr);
        tar.simbad_5 = -999*ones(tar.nT,db.nr);
        inp2.simbad_5  = -999*ones(inp.nT,db2.nr);
        tar2.simbad_5  = -999*ones(tar.nT,db2.nr);
   elseif strcmp(ann.cp,'h12')
        ...
        inp2.simbad_2  = -999*ones(inp.nT,db2.nr);
        tar2.simbad_2  = -999*ones(tar.nT,db2.nr);
    elseif strcmp(ann.cp,'h')
        inp.simbad_1  = -999*ones(inp.nT,2*db.nr);
        tar.simbad_1  = -999*ones(tar.nT,2*db.nr);
        inp2.simbad_1  = -999*ones(inp.nT,2*db2.nr);
        tar2.simbad_1  = -999*ones(tar.nT,2*db2.nr);
    else
        inp.simbad_1  = -999*ones(inp.nT,db.nr);
        tar.simbad_1  = -999*ones(tar.nT,db.nr);
        inp2.simbad_1  = -999*ones(inp.nT,db2.nr);
    ...
    for i_=1:inp.nT
        if strcmp(ann.cp,'h')
            inp.simbad_1(i_,1:2*db.nr) = [log10(PSA_1(1:2*db.nr,inp.idx(i_))./100)'];
            inp2.simbad_1(i_,1:2*db2.nr) = [log10(PSA2_1(1:2*db2.nr,inp2.idx(i_))./100)'];
        else
             inp.simbad_1(i_,1:db.nr) = log10(PSA_1(1:db.nr,inp.idx(i_))./100)';
             inp2.simbad_1(i_,1:db2.nr) = log10(PSA2_1(1:db2.nr,inp2.idx(i_))./100)';
        end
        if strcmp(ann.cp,'h12v')
             inp.simbad_2(i_,1:db.nr) = log10(PSA_2(1:db.nr,inp.idx(i_))./100)';
             inp2.simbad_2(i_,1:db2.nr) = log10(PSA2_2(1:db2.nr,inp2.idx(i_))./100)';

             inp.simbad_5(i_,1:db.nr) = log10(PSA_3(1:db.nr,inp.idx(i_))./100)';
             inp2.simbad_5(i_,1:db2.nr) = log10(PSA2_3(1:db2.nr,inp2.idx(i_))./100)';
        elseif strcmp(ann.cp,'h12')
             inp.simbad_2(i_,1:db.nr) = log10(PSA_2(1:db.nr,inp.idx(i_))./100)';
             inp2.simbad_2(i_,1:db2.nr) = log10(PSA2_2(1:db2.nr,inp2.idx(i_))./100)';
        end
    ...
    for i_=1:tar.nT
        if strcmp(ann.cp,'h')
            tar.simbad_1(i_,1:2*db.nr) = [log10(PSA_1(1:2*db.nr,tar.idx(i_))./100)'];
            tar2.simbad_1(i_,1:2*db2.nr) = [log10(PSA2_1(1:2*db2.nr,tar2.idx(i_))./100)'];
        else
            tar.simbad_1(i_,1:db.nr) = log10(PSA_1(1:db.nr,tar.idx(i_))./100)';
            tar2.simbad_1(i_,1:db2.nr) = log10(PSA2_1(1:db2.nr,tar2.idx(i_))./100)';
        end
        if strcmp(ann.cp,'h12v')
            tar.simbad_2(i_,1:db.nr) = log10(PSA_2(1:db.nr,tar.idx(i_))./100)';
            tar2.simbad_2(i_,1:db2.nr) = log10(PSA2_2(1:db2.nr,tar2.idx(i_))./100)';

            tar.simbad_3(i_,1:db.nr) = log10(PSA_3(1:db.nr,tar.idx(i_))./100)';
            tar2.simbad_3(i_,1:db2.nr) = log10(PSA2_3(1:db2.nr,tar2.idx(i_))./100)';
        elseif strcmp(ann.cp,'h12')
            tar.simbad_2(i_,1:db.nr) = log10(PSA_2(1:db.nr,tar.idx(i_))./100)';
            tar2.simbad_2(i_,1:db2.nr) = log10(PSA2_2(1:db2.nr,tar2.idx(i_))./100)';
        end
    ...
    if strcmp(add_distance,'True')
        ...
        for j_ = 1:db.nr
            if strcmp(ann.cp,'h')
                ...
                inp.simbad_3(index_extra,j_+db.nr) = (max(db.simbad(j_).Rjb,0.01));
            else
                inp.simbad_3(index_extra,j_) =  (max(db.simbad(j_).Rjb,0.01));
            end
        end
        for j_ = 1:db2.nr
            if strcmp(ann.cp,'h')
                inp2.simbad_3(index_extra,j_) =  (max(db2.simbad(j_).Rjb,0.01));
                inp2.simbad_3(index_extra,j_+db2.nr) = (max(db2.simbad(j_).Rjb,0.01));   
            else
                inp2.simbad_3(index_extra,j_) =  (max(db2.simbad(j_).Rjb,0.01));      
            end
        end
    ...
    if strcmp(add_m,'True')
        ...
        for j_ = 1:db.nr
             if strcmp(ann.cp,'h')
                ...
                inp.simbad_3(index_extra,j_+db.nr) =  db.simbad(j_).Mw;      
            else
                inp.simbad_3(index_extra,j_) = db.simbad(j_).Mw;
             end
        end
        for j_ = 1:db2.nr
            if strcmp(ann.cp,'h')
                inp2.simbad_3(index_extra,j_) =  db2.simbad(j_).Mw;
                inp2.simbad_3(index_extra,j_+db2.nr) = db2.simbad(j_).Mw;
            else
                inp2.simbad_3(index_extra,j_) = db2.simbad(j_).Mw;
            end
        end
    ...
    if strcmp(add_lndistance,'True')
        ...
        for j_ = 1:db.nr
            if strcmp(ann.cp,'h')
                inp.simbad_3(index_extra,j_) =  log10(max(db.simbad(j_).Rjb,0.01));
                 inp.simbad_3(index_extra,j_+db.nr) =  log10(max(db.simbad(j_).Rjb,0.01));  
            else
                inp.simbad_3(index_extra,j_) =  log10(max(db.simbad(j_).Rjb,0.01));
            end
        end
        for j_ = 1:db2.nr
            if strcmp(ann.cp,'h')
                inp2.simbad_3(index_extra,j_) =  log10(max(db2.simbad(j_).Rjb,0.01));
                inp2.simbad_3(index_extra,j_+db2.nr) = log10(max(db2.simbad(j_).Rjb,0.01));   
            else
                inp2.simbad_3(index_extra,j_) =  log10(max(db2.simbad(j_).Rjb,0.01));
            end
        end
    ...
    n_classes=0;
     if strcmp(separate_classes,'True')
        ...
        if strcmp(ann.cp,'h')
                inp.simbad_4 = [onehotencode(Class,1),onehotencode(Class,1)];       
            else
       ...
       for j_ = 1:db2.nr
        AA=string(db2.simbad(j_).site_EC8);
        if strcmp(AA,'A*')
        AA='A';
            elseif strcmp(AA,'B*')
        AA='B';
        ...
        Class = categorical(Class);  % ensures categories are known
        if strcmp(ann.cp,'h')
                inp2.simbad_4 = [onehotencode(Class,1),onehotencode(Class,1)];       
            else
                inp2.simbad_4 = onehotencode(Class,1); 
        end
        n_classes = size(inp2.simbad_4,1);
     end
    ...
    for i_=1:dsg.ntr
                ...
                [dsg,layers,idx2] = train_ann_basics_h12(ann,db.nr,inp.nT,tar.nT,dsg,TransferLearning,add_distance,add_m,add_lndistance,separate_classes,n_classes,ann.cp,db2.nr);
    ...
    if strcmp(ann.cp,'h12v')
            if index_extra>0 && n_classes>0
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)',inp2.simbad_2(:,idx2.trn)',inp2.simbad_5(:,idx2.trn)',inp2.simbad_3(:,idx2.trn)',inp2.simbad_4(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)',tar2.simbad_2(:,idx2.trn)',tar2.simbad_3(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)',inp2.simbad_2(:,idx2.vld)',inp2.simbad_5(:,idx2.vld)',inp2.simbad_3(:,idx2.vld)',inp2.simbad_4(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)',tar2.simbad_2(:,idx2.vld)',tar2.simbad_3(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)',inp2.simbad_2(:,idx2.tst)',inp2.simbad_5(:,idx2.tst)',inp2.simbad_3(:,idx2.tst)',inp2.simbad_4(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)',tar2.simbad_2(:,idx2.tst)',tar2.simbad_3(:,idx2.tst)'};
                ...
                dsX2Trn_1 = arrayDatastore(inp2.simbad_1(:,idx2.trn)');
                dsX2Trn_2 = arrayDatastore(inp2.simbad_2(:,idx2.trn)');
                dsX2Trn_5 = arrayDatastore(inp2.simbad_5(:,idx2.trn)');
                dsX2Trn_3 = arrayDatastore(inp2.simbad_3(:,idx2.trn)');
                dsX2Trn_4 = arrayDatastore(inp2.simbad_4(:,idx2.trn)');
                dsT2Trn_1 = arrayDatastore(tar2.simbad_1(:,idx2.trn)');
                dsT2Trn_2 = arrayDatastore(tar2.simbad_2(:,idx2.trn)');
                dsT2Trn_3 = arrayDatastore(tar2.simbad_3(:,idx2.trn)');
                dsTrn2 = combine(dsX2Trn_1,dsX2Trn_2,dsX2Trn_5,dsX2Trn_3,dsX2Trn_4,dsT2Trn_1,dsT2Trn_2,dsT2Trn_3);

                dsX2vld_1 = arrayDatastore(inp2.simbad_1(:,idx2.vld)');
                dsX2vld_2 = arrayDatastore(inp2.simbad_2(:,idx2.vld)');
                dsX2vld_5 = arrayDatastore(inp2.simbad_5(:,idx2.vld)');
                dsX2vld_3 = arrayDatastore(inp2.simbad_3(:,idx2.vld)');
                dsX2vld_4 = arrayDatastore(inp2.simbad_4(:,idx2.vld)');
                dsT2vld_1 = arrayDatastore(tar2.simbad_1(:,idx2.vld)');
                dsT2vld_2 = arrayDatastore(tar2.simbad_2(:,idx2.vld)');
                dsT2vld_3 = arrayDatastore(tar2.simbad_3(:,idx2.vld)');
          ...
          elseif (index_extra>0) && (n_classes==0)
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)',inp2.simbad_2(:,idx2.trn)',inp2.simbad_5(:,idx2.trn)',inp2.simbad_3(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)',tar2.simbad_2(:,idx2.trn)',tar2.simbad_3(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)',inp2.simbad_2(:,idx2.vld)',inp2.simbad_5(:,idx2.vld)',inp2.simbad_3(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)',tar2.simbad_2(:,idx2.vld)',tar2.simbad_3(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)',inp2.simbad_2(:,idx2.tst)',inp2.simbad_5(:,idx2.tst)',inp2.simbad_3(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)',tar2.simbad_2(:,idx2.tst)',tar2.simbad_3(:,idx2.tst)'};
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)',inp2.simbad_2(:,idx2.trn)',inp2.simbad_5(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)',tar2.simbad_2(:,idx2.trn)',tar2.simbad_3(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)',inp2.simbad_2(:,idx2.vld)',inp2.simbad_5(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)',tar2.simbad_2(:,idx2.vld)',tar2.simbad_3(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)',inp2.simbad_2(:,idx2.tst)',inp2.simbad_5(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)',tar2.simbad_2(:,idx2.tst)',tar2.simbad_3(:,idx2.tst)'};
    ...
    elseif strcmp(ann.cp,'h12')
            if index_extra>0 && n_classes>0
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)',inp2.simbad_2(:,idx2.trn)',inp2.simbad_3(:,idx2.trn)',inp2.simbad_4(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)',tar2.simbad_2(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)',inp2.simbad_2(:,idx2.vld)',inp2.simbad_3(:,idx2.vld)',inp2.simbad_4(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)',tar2.simbad_2(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)',inp2.simbad_2(:,idx2.tst)',inp2.simbad_3(:,idx2.tst)',inp2.simbad_4(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)',tar2.simbad_2(:,idx2.tst)'};
                ...
                dsT2Trn_1 = arrayDatastore(tar2.simbad_1(:,idx2.trn)');
                dsT2Trn_2 = arrayDatastore(tar2.simbad_2(:,idx2.trn)');
                dsTrn2 = combine(dsX2Trn_1,dsX2Trn_2,dsX2Trn_3,dsX2Trn_4,dsT2Trn_1,dsT2Trn_2);
                ...
                dsT2vld_1 = arrayDatastore(tar2.simbad_1(:,idx2.vld)');
                dsT2vld_2 = arrayDatastore(tar2.simbad_2(:,idx2.vld)');
                dsVld2= combine(dsX2vld_1,dsX2vld_2,dsX2vld_3,dsX2vld_4,dsT2vld_1,dsT2vld_2);  
          ...
          elseif (index_extra>0) && (n_classes==0)
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)',inp2.simbad_2(:,idx2.trn)',inp2.simbad_3(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)',tar2.simbad_2(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)',inp2.simbad_2(:,idx2.vld)',inp2.simbad_3(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)',tar2.simbad_2(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)',inp2.simbad_2(:,idx2.tst)',inp2.simbad_3(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)',tar2.simbad_2(:,idx2.tst)'};            
                ...
                dsT2Trn_1 = arrayDatastore(tar2.simbad_1(:,idx2.trn)');
                dsT2Trn_2 = arrayDatastore(tar2.simbad_2(:,idx2.trn)');
                dsTrn2 = combine(dsX2Trn_1,dsX2Trn_2,dsX2Trn_3,dsT2Trn_1,dsT2Trn_2);
                ...
                dsT2vld_1 = arrayDatastore(tar2.simbad_1(:,idx2.vld)');
                dsT2vld_2 = arrayDatastore(tar2.simbad_2(:,idx2.vld)');
                dsVld2= combine(dsX2vld_1,dsX2vld_2,dsX2vld_3,dsT2vld_1,dsT2vld_2); 
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)',inp2.simbad_2(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)',tar2.simbad_2(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)',inp2.simbad_2(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)',tar2.simbad_2(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)',inp2.simbad_2(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)',tar2.simbad_2(:,idx2.tst)'};  
                ...
                dsT2Trn_1 = arrayDatastore(tar2.simbad_1(:,idx2.trn)');
                dsT2Trn_2 = arrayDatastore(tar2.simbad_2(:,idx2.trn)');
                dsTrn2 = combine(dsX2Trn_1,dsX2Trn_2,dsT2Trn_1,dsT2Trn_2);
                ...
                dsT2vld_1 = arrayDatastore(tar2.simbad_1(:,idx2.vld)');
                dsT2vld_2 = arrayDatastore(tar2.simbad_2(:,idx2.vld)');
                dsVld2= combine(dsX2vld_1,dsX2vld_2,dsT2vld_1,dsT2vld_2);
            end
            ...
            if index_extra>0 && n_classes>0
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)',inp2.simbad_3(:,idx2.trn)',inp2.simbad_4(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)',inp2.simbad_3(:,idx2.vld)',inp2.simbad_4(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)',inp2.simbad_3(:,idx2.tst)',inp2.simbad_4(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)'};
          ...
          elseif (index_extra>0) && (n_classes==0)
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)',inp2.simbad_3(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)',inp2.simbad_3(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)',inp2.simbad_3(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)'};            
                ...
                NNs{i_}.inp2.trn = {inp2.simbad_1(:,idx2.trn)'};
                NNs{i_}.tar2.trn = {tar2.simbad_1(:,idx2.trn)'};          
                NNs{i_}.inp2.vld = {inp2.simbad_1(:,idx2.vld)'};
                NNs{i_}.tar2.vld = {tar2.simbad_1(:,idx2.vld)'};            
                NNs{i_}.inp2.tst = {inp2.simbad_1(:,idx2.tst)'};
                NNs{i_}.tar2.tst = {tar2.simbad_1(:,idx2.tst)'};  
...
elseif strcmp(ann.cp,'h12')
     ...
     lossFcn = @(Y1,T1) 1*mse(Y1,T1) ;
                   ...
                   layerName = ["fc_shared1","fc_shared2","output1","output2","input4"];
                   layers = freezeNetwork(layers,LayerNamesToIgnore=layerName);

                   % Options for second training
                   options = trainingOptions('adam', ...    % Adam optimization
    'InitialLearnRate', 0.003, ...                 % Set the initial learning rate to 0.01 
    'MaxEpochs', 900, ...                         % Maximum number of epochs to train algorithm
    'ValidationData', dsVld2, ...        % Dataset to use as the validation set
     'MiniBatchSize', 30, ...
    'Shuffle', 'every-epoch', ...
    'ValidationFrequency', 3,...
    'ValidationPatience',7,...        %early stopping taking the validation loss as reference
    'Verbose', false, ...                         % Outputs information about training
    'OutputNetwork','best-validation-loss',...
    'L2Regularization', 0.002...
      );       

                   [NNs{i_}.net,NNs{i_}.trs] = trainnet(dsTrn2,layers,lossFcn,options);
    ...
    trann_train_psa_performance(NNs,inp2,tar2,wd,dsg,dbn_name,verNet,ann.cp,TransferLearning);

    %% *OUTPUT*
       if strcmp(add_distance,'True') && strcmp(add_m,'True') && strcmp(add_lndistance,'True') && strcmp(separate_classes,'True')
         save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'Rjb_Mw_logRjb_Site',dbn_name,'TL',verNet)),'NNs');  
       elseif strcmp(add_m,'True') && strcmp(add_lndistance,'True') && strcmp(separate_classes,'True')
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'Mw_logRjb_Site',dbn_name,'TL',verNet)),'NNs');
      elseif strcmp(add_distance,'True') && strcmp(add_m,'True') && strcmp(add_lndistance,'True')
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'Rjb_Mw_logRjb',dbn_name,'TL',verNet)),'NNs'); 
       elseif strcmp(add_distance,'True') && strcmp(add_m,'True')
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'Rjb_Mw',dbn_name,'TL',verNet)),'NNs'); 
       elseif strcmp(add_distance,'True') && strcmp(add_lndistance,'True')
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'Rjb_logRjb',dbn_name,'TL',verNet)),'NNs');
       elseif strcmp(add_m,'True') && strcmp(add_lndistance,'True')
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'Mw_logRjb',dbn_name,'TL',verNet)),'NNs'); 
       elseif strcmp(add_distance,'True')
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'Rjb',dbn_name,'TL',verNet)),'NNs'); 
       elseif strcmp(add_m,'True')
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'Mw',dbn_name,'TL',verNet)),'NNs');
       elseif strcmp(add_lndistance,'True')
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,'logRjb',dbn_name,'TL',verNet)),'NNs');
       else
        save(fullfile(wd,sprintf('net_%u_%s_%s_%s%s_%s.mat',...
        round(ann.TnC*100),ann.scl,ann.cp,dbn_name,'TL',verNet)),'NNs');
       end
